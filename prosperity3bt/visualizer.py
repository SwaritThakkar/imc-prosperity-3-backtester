...
def plot_dashboard(
    ...
    # Level 3 prices (faintest) — plot first so they sit behind everything
    for side, col, clr in [
        ...
        s = _col(prices, col)
        if s is not None:
            ax1.plot(ts, s, color=clr, linewidth=0.6, alpha=0.4,
                     label=f"L3 {side}")

    # Level 2 prices (faint)
    for side, col, clr in [
        ...
        s = _col(prices, col)
        if s is not None:
            ax1.plot(ts, s, color=clr, linewidth=0.8, alpha=0.45,
                     label=f"L2 {side}")

    # Best bid and best ask (slightly more visible)
    s_bid = _col(prices, "bid_price_1")
    s_ask = _col(prices, "ask_price_1")
    if s_bid is not None:
        ax1.plot(ts, s_bid, color=CLR["best_bid"], linewidth=0.9,
                 alpha=0.55, label="Best Bid")
    if s_ask is not None:
        ax1.plot(ts, s_ask, color=CLR["best_ask"], linewidth=0.9,
                 alpha=0.55, label="Best Ask")
    ...
    if bb_u is not None and bb_l is not None:
        ax1.fill_between(ts, bb_l, bb_u, color=CLR["bb_fill"],
                         label="BB Band", zorder=2)
        ax1.plot(ts, bb_u, color=CLR["bb_upper"], linewidth=1.0,
                 linestyle="--", label="BB Upper", zorder=3)
        ax1.plot(ts, bb_l, color=CLR["bb_lower"], linewidth=1.0,
                 linestyle="--", label="BB Lower", zorder=3)

    # Bollinger mean (SMA)
    s_bb_mean = _col(prices, "bb_mean")
    if s_bb_mean is not None:
        ax1.plot(ts, s_bb_mean, color=CLR["bb_mean"], linewidth=1.1,
                 linestyle="-.", label=f"BB Mean (SMA {BB_PERIOD})", zorder=4)

    # EMA
    s_ema = _col(prices, "ema")
    if s_ema is not None:
        ax1.plot(ts, s_ema, color=CLR["ema"], linewidth=1.5,
                 label=f"EMA ({EMA_PERIOD})", zorder=5)

    # Mid price — primary line, plotted on top
    s_mid = _col(prices, "mid_price")
    if s_mid is not None:
        ax1.plot(ts, s_mid, color=CLR["mid"], linewidth=1.2,
                 label="Mid Price", zorder=6)

    # ── Trade scatter overlay ─────────────────────────────────────────────────
    if not trades.empty:
        # Determine trade direction from buyer/seller fields when possible.
        # Convention: if buyer is empty or "SUBMISSION" it's a market sell order
        # triggering (price went to ask) — mark as buy (bought at ask).
        # If seller is "SUBMISSION" it's a market buy (sold at bid) — mark sell.
        # If neither is deterministic, mark as neutral.

        def _classify_trade(row):
            buyer  = str(row.get("buyer",  "")).strip().upper()
            seller = str(row.get("seller", "")).strip().upper()
            if buyer  == "SUBMISSION":
                return "sell"    # we submitted the sell; counterparty bought
            if seller == "SUBMISSION":
                return "buy"     # we submitted the buy
            return "neutral"

        trades_copy = trades.copy()
        trades_copy["direction"] = trades_copy.apply(_classify_trade, axis=1)

        for direction, color, marker, label in [
            ("buy",     CLR["buy_trade"],     "^", "Buy Trade"),
            ("sell",    CLR["sell_trade"],    "v", "Sell Trade"),
            ("neutral", CLR["neutral_trade"], "o", "Trade"),
        ]:
            sub = trades_copy[trades_copy["direction"] == direction]
            if sub.empty:
                continue
            ax1.scatter(
                sub["timestamp"], sub["price"],
                c=color, marker=marker, s=35, zorder=7,
                label=label, edgecolors="none", alpha=0.85,
            )
